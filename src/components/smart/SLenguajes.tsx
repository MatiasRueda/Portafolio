...
export default function SLenguajes(): JSX.Element {
  ...
  return (
    <div className="cont-lenguajes">  
      {rutasHerramientas.filter(herramienta => 
        (herramienta.tipo === TIPO.TODOS || herramienta.tipo?.toLowerCase() === tipo.toLowerCase()))
                          .map(herramienta => 
                              <Link key={herramienta.nombre} 
                                    style={nombre.toLowerCase() === herramienta.nombre.toLowerCase()? elegido : noElegido}
                                    href={`${herramienta.direccion + (herramienta.tipo === TIPO.TODOS? "/"+ partes[2] : "")}`}>
                                    {herramienta.nombre}
                              </Link>)}
    </div>
  )
